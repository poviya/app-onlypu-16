...
import MyReactComponent from '../zego-cloud/myReactComponents';
...
function generateToken(tokenServerUrl: string, userID: string) {
  ...
  return fetch(
    `${tokenServerUrl}/access_token?userID=${userID}&expired_ts=7200000`,
    {
      method: 'GET',
    }
  ).then((res) => res.json());
...
@Component({
  ...
  styleUrls: ['./zego-cloud.component.scss']
})
export class ZegoCloudComponent implements OnInit {
