...
export class MainComponent implements OnInit {
  ...
  findAllPosts(): void {
    ...
    this.camService.findAllActive(data, this.limitPage, this.currentPage).subscribe(res => {
      if (res) {
        this.totalPages = res.totalPages;
        this.dataSearch = data;
        res.data.forEach((element: any) => {
          this.cams.push(element);
        });
        this.loading = false;
        this.headPage();
      }
  ...
  findAllInfinite() {
    if (this.currentPage < this.totalPages) {
      ...
      this.camService.findAllActiveInfinite(this.dataSearch, this.limitPage, this.currentPage).subscribe(res => {
        if (res) {
          res.data.forEach((element: any) => {
            this.cams.push(element);
          });
